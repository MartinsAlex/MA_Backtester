    ...
    def analyse(self):
        ...
        resultsTable = pd.DataFrame(
            columns=["From", "To", "Ticker", "Number of Trades", "Winning Trades", "Losing Trades",
                     ...
                     "Asset Max Drawdown", "Strategy Max Drawdown", "Market Exposure", "99 % Strategy Annual VaR", "99 % Asset Annual VaR",
        ...
        for symbol in self.tickers:  # On commence la boucle qui va itérer sur chaque symbol dans la liste
            ...
            if symbol in list(forexFREDlist):  # Forex
                ...
                except (KeyError, OverflowError, RemoteDataError):  # Si il y a erreur on regarde si le problème vient de la date
                ...
                #try:
                #        self.__AssetDfTest = data.get_data_yahoo(symbol, interval='d')  # We download the entire dataFrame
#
                #        firstDate = self.__AssetDfTest.index[0]  # On prend la date minimale
                #        if firstDate > self.endDate:
                #            print("There isnt data before the {} for {}".format(firstDate, symbol))
                #            continue
#
...
#
            ...
            if nbTrades > 0:  # If at least 1 trade has been made
                ...
                balanceFinale = self.portfolioBalanceEvolution[-(holdingDays+1)] # Without the open position
                if assetPercentPerf < 0 and (balanceFinale / self.initialBalance - 1) > 0:
                    resultsTable.loc[x, ["Over/Under-performance %"]] = abs(assetPercentPerf) + (balanceFinale / self.initialBalance - 1)
                elif assetPercentPerf > 0 and (balanceFinale / self.initialBalance - 1) < 0:
                    resultsTable.loc[x, ["Over/Under-performance %"]] = (balanceFinale / self.initialBalance - 1) - assetPercentPerf
                elif assetPercentPerf > 0 and (balanceFinale / self.initialBalance - 1) > 0:
                    resultsTable.loc[x, ["Over/Under-performance %"]] = (balanceFinale / self.initialBalance - 1) - assetPercentPerf
                else: # both < 0
                    if assetPercentPerf > (balanceFinale / self.initialBalance - 1):
                        #resultsTable.loc[x, ["Over/Under-performance %"]] = assetPercentPerf - (balanceFinale / self.initialBalance - 1)
                        resultsTable.loc[x, ["Over/Under-performance %"]] = (balanceFinale / self.initialBalance - 1) - assetPercentPerf
                ...
                if self.portfolioBalanceEvolution[-1] < 1:  # Si on s'est carrément endetté avec une position short, le rdt est nulle
                    ...
                    resultsTable.loc[x, ["Strategy Annualized Return %"]] = (balanceFinale / self.initialBalance) ** (tradingDaysNb / totalDays) - 1
                ...
                resultsTable.loc[x, ["99 % Strategy Annual VaR"]] = (np.mean(portfolioReturns) + st.norm.ppf(1-0.01) * np.std(portfolioReturns))*(tradingDaysNb**0.5)

                resultsTable.loc[x, ["99 % Asset Annual VaR"]] = (np.mean(self.AssetDf["Log_Returns"]) + st.norm.ppf(1-0.01) * np.std(self.AssetDf["Log_Returns"]))*(tradingDaysNb**0.5)
            ...
            if symbol in list(forexFREDlist):  # Le volume n'est pas fourni par FRED pour le forex
