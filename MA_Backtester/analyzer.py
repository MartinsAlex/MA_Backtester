    ...
    def analyse(self):
        ...
        for symbol in self.tickers:  # On commence la boucle qui va itérer sur chaque symbol dans la liste
            ...
            if nbTrades > 0:  # If at least 1 trade has been made
                ...
                resultsTable.loc[x, ["99 % Strategy Daily VaR"]] = (np.mean(portfolioReturns) + st.norm.ppf(1-0.01) * np.std(portfolioReturns))#*(tradingDaysNb**0.5)

                resultsTable.loc[x, ["99 % Asset Daily VaR"]] = (np.mean(self.AssetDf["Log_Returns"]) + st.norm.ppf(1-0.01) * np.std(self.AssetDf["Log_Returns"]))#*(tradingDaysNb**0.5)
