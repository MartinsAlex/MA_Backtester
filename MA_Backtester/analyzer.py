...
class strategyDefiner:
    ...
    * Key-worded args :
    ...
    def analyse(self):
        ...
        # Full list of forex tickers (to recognize them later)
        forexFREDlist =  ['DEXUSAL', 'DEXBZUS', 'DEXUSUK', 'DEXCAUS', 'DEXCHUS', 'DEXDNUS', 'DEXUSEU',
                          'DEXHKUS', 'DEXINUS','DEXJPUS','DEXMAUS','DEXMXUS','DEXTAUS','DEXUSNZ',
                          'DEXNOUS','DEXSIUS', 'DEXSFUS', 'DEXKOUS','DEXSLUS','DEXSDUS','DEXSZUS','DEXTHUS']
        ...
        for symbol in self.tickers:  # We begin the boucle that will iterate on every tickers
            ...
            if symbol[-4:] == "-USD": # Crypto symbol have symbol with "-usd" at the end so we recognize them like this
                tradingDaysNb = 365  # Because a crypto is traded all the days of the year
            elif symbol in forexFREDlist:
                tradingDaysNb = 262 # FRED forex prices have 262 days per year
            else:
            ...
            if nbTrades > 0:  # If at least 1 trade has been made
                ...
                resultsTable.loc[x, ["Win Rate"]] = round(winRate, 4)
                ...
                if self.portfolioBalanceEvolution[-1] < 0:  # If we went into debt because of a short position, the return is null.
                    ...
                    resultsTable.loc[x, ["Strategy Annualized Return %"]] = (finalBalance / self.initialBalance) ** (tradingDaysNb / len(self.AssetDf)) - 1
    ...
    def seasonalityTable(self, ticker=None):
        ...
        Return a pandas pivot table with the total profits & losses for each years & months of the strategy.
        ...
        if ticker != None:
            seasonTable = self.transactionTable[self.transactionTable["Ticker"] == ticker] # The user can specify a ticker if the analysis has been done on several
        else:
            seasonTable = self.transactionTable  # We create a copy of the transaction table

        seasonTable['Total P/L'] = seasonTable['P/L'].astype(float)*seasonTable['Number of shares'].astype(float)
        ...
        seasonTable = pd.pivot_table(seasonTable, index=seasonTable["Month"], columns=seasonTable['Year'], values='Total P/L', aggfunc=np.mean, fill_value=0)
    ...
    def optimize(self, indicator, fastMaRange=[], slowMaRange=[], type="max"):
        ...
        for fastDays in range(fastMaRange[0], fastMaRange[1] + 1):
            for slowDays in range(slowMaRange[0], slowMaRange[1] + 1):
                ...
                test = strategyDefiner(self.tickers, fastDays, slowDays, self.beginDate, self.endDate, shortLong=self.shortLong, capital=1_000_000, maType=self.maType,
