...
class movingAverageCrossover:
    '''
    The class for testing moving average crossover strategy.
    ...
    Attributes :
        ...
        1. tickers (list): The ticker list on which we test the strategy (example : ["AAPL", "TSLA", "AMZN", "GOOG"])
        ...
        shortLong (str), 3 possibilities :
            "long" : long only
            "short" : short only
            "both" : long and short
        capital (int or float) : The capital invested in the begin of the strategy
        ...
        stopLoss (float, from 0 to 1) : the limit for selling positon, in order to reduce losses.
            Note that : because the analysis is made on Adjusted close prices (at the end of trading days) your loss can exceed the stop-loss.
                        (example : stop-loss of 5 %, today price of a stock is 100. If tomorrow the price drop to 90, the share will be sold and
                        the loss will be 10, a 10 % loss)
        takeProfit (float) : the limit for selling positon, in order to confirm profit.
        rf (float) : if you want to specify the risk-free rate for the statistics computations. If not, the default risk-free rate will be the
                     13 Week Treasury Bill rate of the day.
        makeplot (bool) : If True, the algorithm will make a plot of your strategy behavior. The base value is False and is recommended when your work on
                          multiple stocks (5 or more).

    '''

    def __init__(self, tickers, fastMA, slowMA, beginDate, endDate, shortLong=str, capital=1_000_000, maType=str,
                 **kwargs):
    ...
    def analyse(self):
        ...
        """
        The method that realize the strategy analysis. It is run by default when the class is called.
        ...
        resultsTable = pd.DataFrame(
            columns=["From", "To", "Ticker", "Number of Trades", "Winning Trades", "Losing Trades",
                     ...
                     "Buy & Hold Return", "Asset Return %", "Strategy Realized Return %", "Strategy Max Drawdown",
                     "Asset Max Drawdown",
        ...
        forexFREDlist = ['DEXUSAL', "DEXBZUS", "DEXUSUK", "DEXCAUS", "DEXCHUS", "DEXDNUS", "DEXUSEU", "DEXHKUS",
                         "DEXINUS", "DEXJPUS", "DEXMAUS", "DEXMXUS", "DEXTAUS", "DEXUSNZ", "DEXNOUS", "DEXSIUS",
                         "DEXSFUS",
        ...
        for symbol in self.tickers:  # On commence la boucle qui va itérer sur chaque symbol dans la liste
            ...
            if symbol in forexFREDlist:  # On vérifie si le symbol est une monnaie (s'il est dans la liste des symbols forex)
                df = data.get_data_fred(symbol, self.beginDate,
                                        self.endDate)  # On effectue la requête avec la fonction de pandas_datareader
            ...
            df["Log_Returns"] = np.log(1 + df[priceColumn].pct_change(1))
            ...
            portfolioVariations = [] # Pour le graphique qui compare le portefeuille à l'actif
            ...
            for i in range(1, len(df[priceColumn])):  # On doit commencer à 1 car sinon on peut pas comparer à la période précédente
                ...
                if longPrice > 0:  # Si on a une position long ouverte, on enregistre :
                    ...
                    portfolioVariations.append(df["Log_Returns"].iloc[i])
                    ...
                    portfolioVariations.append(-df["Log_Returns"].iloc[i])  # Si on a une position short notre portefeuille évolue inversément au rendement journalier

                else:
                    portfolioVariations.append(0) # Si on a pas de positions le portefeuille ne subit aucune variation
                ...
                if (df[shortMaCol].iloc[i - 1] > df[longMaCol].iloc[i - 1] and df[shortMaCol].iloc[i] <
                        df[longMaCol].iloc[i]):  # le i ne doit pas dépasser la longueur de la df
                    ...
                    if self.shortLong == "both" or self.shortLong == "long":  # Cette partie ne concerne que le long
                        ...
                        if longPrice > 0:
                            ...
                            # Win & Loss stats
                            winTrade, looseTrade, gains, lost = self.__winLossRatio(
                                (df[priceColumn].iloc[i] - longPrice),
                                winTrade, looseTrade, gains, lost)
                    ...
                    if (self.shortLong == "both" or self.shortLong == "short") and shortPrice == 0:
                        ...
                        if ((capit / df[priceColumn].iloc[
                            i]) < 1):  # Malgrès que le short ne nécessite pas de fonds on ne prend pas de position supérieur aux fonds qu'on possède pour éviter le sur-endettemment
                            # print("Fonds insuffisant")
                            continue
                ...
                if self.stopLoss != None:
                    ...
                    if longPrice > 0:
                        ...
                        if (longPrice / df[priceColumn].iloc[i] - 1) >= stopLoss:
                            ...
                            # Win & Loss stats
                            winTrade, looseTrade, gains, lost = self.__winLossRatio(
                                (df[priceColumn].iloc[i] - longPrice), winTrade, looseTrade, gains, lost)
                    ...
                    if shortPrice > 0:
                        if (df[priceColumn].iloc[i] / shortPrice - 1) >= stopLoss:  # Si la perte dépasse celle indiquée
                            ...
                            # Win & Loss stats
                            winTrade, looseTrade, gains, lost = self.__winLossRatio(
                                (shortPrice - df[priceColumn].iloc[i]), winTrade, looseTrade, gains, lost)
            ...
            if nbTrades > 0:  # Si au moins 1 trade a été effectué
                ...
                resultsTable.loc[x, ["Strategy Realized Return %"]] = capit / self.capital - 1
                ...
                if capit < 1:  # Si on s'est carrément endetté avec une position short, on ne peut pas calculer le rdt annualisé
                    ...
                    resultsTable.loc[x, ["Strategy Annualized Return %"]] = (capit / self.capital) ** (
                            252 / float(len(df))) - 1
            ...
            assetRt = df["Log_Returns"].cumsum()  # Somme cumulés des rendements logarithmiques de l'actif
            assetDiscRt = np.exp(assetRt)  # Transformations en rendements discrets
            buyAndHoldcapital = self.capital * assetDiscRt  # Evolution d'un investissemnt "buy and hold"

            portfolioVariations = pd.Series(portfolioVariations).cumsum()  # Somme cumulés des rendements logarithmiques du capital de la stratégie
            portfolioDiscVariations = np.exp(portfolioVariations)  # Transformations en rendements discrets
            stratcapital = self.capital * portfolioDiscVariations  # Evolution du capital du portefeuille

            resultsTable.loc[x, ["Correlation with underlying"]] = np.corrcoef(assetRt[1:], portfolioVariations)[0,1]
            ...
            resultsTable.loc[x, ["Asset Sharpe Ratio"]] = (252 * np.mean(df["Log_Returns"]) - self.rf) / (np.std(df["Log_Returns"]) * (252 ** 0.5))

            resultsTable.loc[x, ["Strategy Sharpe Ratio"]] = (252 * np.mean(stratReturns) - self.rf) / (np.std(stratReturns) * (252 ** 0.5))  # Rdt annualisé
            ...
            if self.__makeplot == True:
                ...
                ax2.plot(df.index[1:], stratcapital, label="Portfolio", linewidth=0.8, color="red")
    ...
    def showPlot(self):
        """
        Allows you to visualize the plot(s) that has been made.
    ...
    def seasonalityTable(self, ticker=None):
        ...
        if ticker != None:
            transacDf = self.transactionTable[self.transactionTable["Ticker"] == ticker]
        else:
        ...
        transacDf = pd.pivot_table(transacDf, index=transacDf["Month"], columns=transacDf['Year'],
                                   values='P/L', aggfunc=np.mean, fill_value=0)
        return transacDf
...
stats = movingAverageCrossover(["AMZN"], 100, 400, "1990-1-1", dt.datetime.today(), maType="simple",
                               makeplot=True, shortLong="long", capital=1_000_000)
#
stats.showPlot()
print(stats.resultsTable)
