...
class movingAverageCrossover:
    ...
    * Key-worded args :
    ...
    def analyse(self):
        ...
        forex_list = pkg_resources.resource_filename('MA_Backtester', 'tickerLists/forex_tickers.csv')
        crypto_list = pkg_resources.resource_filename('MA_Backtester', 'tickerLists/crypto_tickers.csv')

        # Read the csv list of Forex Tickers
        forexFREDlist = pd.read_csv(str(forex_list), sep=";")
        ...
        for symbol in self.tickers:  # We begin the boucle that will iterate on every tickers
            ...
            cryptoTickerList = pd.read_csv(str(crypto_list), sep=";")
