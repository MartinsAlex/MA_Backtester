...
def MA_CROSS(tickerList, fastSma, slowSma, beginDate, endDate, ma=str, showplot=bool,
             stop=[None, None], saveplot=False, rf=None, shortLong=str, capital=1_000_000):
    ...
    for symbol in tickerList: # On commence la boucle qui va itérer sur chaque symbol dans la liste
        ...
        # Weighted Moving Average
            ...
            df[moyenneAcronym + str(fastSma)] = df[priceColumn].rolling(fastSma).apply(
                lambda prices: np.dot(prices, weights1) / weights1.sum(), raw=True) # On multiplie chaque prix par le poids qui lui est attribué, puis on divise pour obtenir la moyenne pondérée
        ...
        if showplot == True:
            ...
            if moyenneAcronym == "EMA":
                fig.suptitle("Exponential MA cross strategy {} for {}".format(slTitre, symbol), size=20)
            elif moyenneAcronym == "SMA":
                fig.suptitle("Simple MA cross strategy {} for {}".format(slTitre, symbol), size=20)
            else:
                fig.suptitle("Weighted MA cross strategy {} for {}".format(slTitre, symbol), size=20)
